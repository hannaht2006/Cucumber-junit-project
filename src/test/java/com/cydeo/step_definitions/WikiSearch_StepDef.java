...
public class WikiSearch_StepDef {
 ...
 Driver.getDriver().get("https://www.wikipedia.org/");
    ...
    public void userSeesIsInTheWikiTitle(String string) {
       Assert.assertTrue(Driver.getDriver().getTitle().startsWith(string) );
    }

    @Then("User sees {string} is in the main header")
    public void userSeesIsInTheMainHeader(String string) {
    String actualHeaderText = wikiSearchPage.header.getText();
    Assert.assertTrue(actualHeaderText.contains(string));
    }

    @Then("User sees {string} is in the image header")
    public void userSeesIsInTheImageHeader(String string) {

        String imageHeaderText = wikiSearchPage.imageHeader.getText();
        Assert.assertTrue(imageHeaderText.contains(string));
    }
}
